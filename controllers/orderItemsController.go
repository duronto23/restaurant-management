...
var orderItemCollection *mongo.Collection = database.OpenConnection(database.Client, "orderItem")
...
func GetOrderItems() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while listing onder items"})
			return
		}
...
func GetOrderItemsByOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		allOrderItems, err := ItemsByOder(orderId)
...
func UpdateOederItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
